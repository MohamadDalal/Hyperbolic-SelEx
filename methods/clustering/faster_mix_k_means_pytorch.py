...
class K_Means:
    ...
    def __init__(self, k=3, tolerance=1e-4, max_iterations=100, init='k-means++',
                 n_init=10, random_state=None, n_jobs=None, pairwise_batch_size=None, mode=None, protos=None,cluster_size=None,is_code=False):
        self.k = k
    ...
    def kpp(self, X, pre_centers=None, k=10, random_state=None):
        ...
        while C.shape[0] < k:
            ...
            if ind >=X.shape[0]:
                C = torch.cat((C, X[-1].view(1, -1)), dim=0)  #
            else:
                C = torch.cat((C, X[ind].view(1, -1)), dim=0)#
            if self.cluster_size is not None and nsamples==0:
                if dist[ind].shape[0]>self.cluster_size:
                    ignored = torch.argsort(dist[ind], dim=-1)[:self.cluster_size]
                    index = np.array(range(X.size(0)))  # get dim 0 index
                    del_index = ignored.numpy().astype(int)  # the raw you want to delete
                    new_index = np.delete(index, del_index[del_index<=index[-1]], axis=0)
                    X = X[new_index, :]
    ...
    def fit_mix_once(self, u_feats, l_feats, l_targets, random_state):
        ...
        #initialize the centers, the first 'k' elements in the dataset will be our initial centers
        if self.protos is None: centers = self.kpp(u_feats, l_centers, k=self.k, random_state=random_state)
        else: centers=torch.from_numpy(self.protos).to(device)
        centers[:len(l_classes)] = l_centers
        ...
        for it in range(self.max_iterations):
            ...
            u_labels=u_labels
            labels[l_num:]=u_labels.to(device)
                    ...
                    for i in range(it):
                        for idx in range(self.k):
                            ...
                            if len(u_selected.size())==0 or u_selected.shape[0]<num:
                                ...
                                indexes =u_selected[indexes].to(device)
                                cluster_free_unseen[indexes]=True
                                sparse_cluster[idx]=False
                            else:
                                sparse_cluster[idx]=False
                        cluster_distance=torch.argsort(dist[cluster_free_unseen.cpu(), :][:, sparse_cluster], dim=1)
